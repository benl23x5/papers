...
__global__ void fused9Kernel(MAXALIGN *data, int dst, int s0, int s1, int s2, int s3, int s4, int s5, int s6, int len, int scratch) {
  int address = blockDim.y * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
  if (address < len) {
    float *pDst = (float*)(&data[dst]);
    int *pSrc0 = (int*)(&data[s0]);
...
__global__ void fused10Kernel(MAXALIGN *data, int dst, int s0, int len, int scratch) {
  int address = blockDim.y * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
  if (address < len) {
...
__global__ void fused18Kernel(MAXALIGN *data, int dst, int s0, int s1, int len, int scratch) {
  int address = blockDim.y * blockIdx.y + blockDim.x * blockIdx.x + threadIdx.x;
  if (address < len) {
    float *pDst = (float*)(&data[dst]);
    int *pSrc0 = (int*)(&data[s0]);
...
void fused9(vec_p d, vec_p s0, vec_p s1, vec_p s2, vec_p s3, vec_p s4, vec_p s5, vec_p s6, int len, vec_p scratch) {
  if (len==0) {return;}
  SYNC();
  DEF_BLOCKS_PER_GRID(len);
  fused9Kernel<<<blocksPerGrid, THREADS_PER_BLOCK>>>(ComputeMemory, d, s0, s1, s2, s3, s4, s5, s6, len, scratch);
  cutilCheckMsg("fused9 execution failed\n");
}

void fused10(vec_p d, vec_p s0, int len, vec_p scratch) {
  if (len==0) {return;}
  SYNC();
  DEF_BLOCKS_PER_GRID(len);
  fused10Kernel<<<blocksPerGrid, THREADS_PER_BLOCK>>>(ComputeMemory, d, s0, len, scratch);
...
void fused13(vec_p d, vec_p s0, vec_p s1, vec_p s2, vec_p s3, vec_p s4, vec_p s5, vec_p s6, vec_p s7, vec_p s8, vec_p s9, vec_p s10, vec_p s11, vec_p s12, vec_p s13, vec_p s14, vec_p s15, vec_p s16, vec_p s17, vec_p s18, vec_p s19, vec_p s20, vec_p s21, vec_p s22, vec_p s23, vec_p s24, vec_p s25, vec_p s26, vec_p s27, vec_p s28, vec_p s29, vec_p s30, vec_p s31, vec_p s32, vec_p s33, vec_p s34, vec_p s35, vec_p s36, vec_p s37, vec_p s38, int len, vec_p scratch) {
  if (len==0) {return;}
  SYNC();
  DEF_BLOCKS_PER_GRID(len);
  fused13Kernel<<<blocksPerGrid, THREADS_PER_BLOCK>>>(ComputeMemory, d, s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16, s17, s18, s19, s20, s21, s22, s23, s24, s25, s26, s27, s28, s29, s30, s31, s32, s33, s34, s35, s36, s37, s38, len, scratch);
...
void fused16(vec_p d, vec_p s0, vec_p s1, vec_p s2, vec_p s3, vec_p s4, vec_p s5, vec_p s6, vec_p s7, vec_p s8, vec_p s9, vec_p s10, vec_p s11, vec_p s12, vec_p s13, vec_p s14, vec_p s15, vec_p s16, vec_p s17, vec_p s18, vec_p s19, vec_p s20, vec_p s21, vec_p s22, vec_p s23, vec_p s24, vec_p s25, vec_p s26, vec_p s27, vec_p s28, vec_p s29, vec_p s30, vec_p s31, vec_p s32, vec_p s33, vec_p s34, vec_p s35, vec_p s36, vec_p s37, vec_p s38, vec_p s39, int len, vec_p scratch) {
  if (len==0) {return;}
  SYNC();
  DEF_BLOCKS_PER_GRID(len);
  fused16Kernel<<<blocksPerGrid, THREADS_PER_BLOCK>>>(ComputeMemory, d, s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16, s17, s18, s19, s20, s21, s22, s23, s24, s25, s26, s27, s28, s29, s30, s31, s32, s33, s34, s35, s36, s37, s38, s39, len, scratch);
...
void fused18(vec_p d, vec_p s0, vec_p s1, int len, vec_p scratch) {
  if (len==0) {return;}
  SYNC();
  DEF_BLOCKS_PER_GRID(len);
  fused18Kernel<<<blocksPerGrid, THREADS_PER_BLOCK>>>(ComputeMemory, d, s0, s1, len, scratch);
...
make_inplace(fused9, INPLACE_4)
make_inplace(fused10, INPLACE_NONE)
make_inplace(fused11, INPLACE_NONE)
make_inplace(fused12, INPLACE_NONE)
make_inplace(fused13, INPLACE_4)
make_inplace(fused14, INPLACE_1)
make_inplace(fused15, INPLACE_1)
make_inplace(fused16, INPLACE_5)
make_inplace(fused17, INPLACE_1)
make_inplace(fused18, INPLACE_2)
make_inplace(fused19, INPLACE_2)
...
fused OP7#1 ($0 : FLOAT, $1 : FLOAT) = (* FLOAT @ $0 (ATAN @ $1))
fused OP8#1 ($0 : FLOAT, $1 : SEGDES, $2 : FLOAT, $3 : FLOAT) = (SELECT FLOAT @
  (> FLOAT @ $0 (DIST FLOAT @ (CONST FLOAT 0.0) $1)) $2 $3)
fused OP9#1 ($0 : SEGDES, $1 : SEGDES, $2 : SEGDES, $3 : FLOAT, $4 : SEGDES, $5 : FLOAT, $6 : FLOAT) = (/ FLOAT @
...
fused OP17#1 ($0 : INT, $1 : INT, $2 : INT) = (= INT @ (- INT @ $0 $1) $2)
fused OP18#1 ($0 : SEGDES, $1 : FLOAT) = (- FLOAT @ (DIST FLOAT @ (CONST FLOAT 1.0) $0) $1)
fused OP19#1 ($0 : SEGDES, $1 : FLOAT) = (- FLOAT @ (DIST FLOAT @ (CONST FLOAT 0.0) $0) $1)
fused OP20#1 ($0 : INT, $1 : FLOAT) = (* FLOAT @ (EXP @ (I_TO_F @ $0)) (LOG @ $1))
*/
