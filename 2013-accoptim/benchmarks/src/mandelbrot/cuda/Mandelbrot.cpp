...
void runBenchmark(int argc, char **argv)
{
    ...
    // Allocate memory for the host simulation
...
#else
        RunMandelbrot0(d_dst, imageW, imageH, crunch, x, y,
                xJParam, yJParam, s, colors, pass++, animationFrame, 0, numSMs, g_isJuliaSet, version);
        cudaDeviceSynchronize();
#endif
    }
    ...
    printf("\n");
    printf("MegaPixels Per Second %.4f\n",PixelsPerSecond/1e6);
    printf("Mean time per frame: %f ms\n", ExecutionTime / N);
