...
In our concrete implementation we also include a @drainS@ function so that the same process could be evaluated sequentially if desired. The definition of @drainS@ is similar to @drainP@, except that we evaluate each stream in turn rather than concurrently.
...
The first form is a source transformer, taking a source of elements of type @a@ and returning a source of elements of type @b@. The second form is a sink transformer, taking a sink of elements of type @b@ and returning a sink of elements of type @a@. The polarity diagram for both forms is given in Figure~\ref{f:Map}. Note that in both cases the elements have type @a@ and the output elements have type @b@.

The definition of @map_i@ is as follows. We have written this definition using separate let-bindings for each intermediate function, rather than with a single inline lambda abstraction for two reasons:
\begin{code}
map_i :: (a -> b) -> Sources i m a -> Sources i m b
...
Note that both @map_i@ and @map_o@ are simply flow transformers, and are neither inherently parallel or sequential. Repa Flow supports data parallelism, but parallelism is introduced by the singular @drainP@ function. No other combinator library need be concerned about it. In this respect Repa Flow operates similarly to the delayed arrays approach of our original Repa library \CITE, except that parallelism is introduced on a per-stream level rather than a per-element level.
...
So for the first stream the segment lengths are @[3 2 1]@ and the elements are @[1 2 3 1 1 5]@, we sum up the first three elements, then the next two, then the next one, yielding @[6 2 5]@ for that stream. When @drainP@ from Figure~\ref{f:Draining} is applied to the result source then each of the individual streams in the flow is folded in parallel. We include the definition of @folds_iii@ and the types and definition of the other versions in the appendix.

With @folds_iii@ we assume that both the segment lengths and elements are available as sources. When this is true, evaluation of @folds_iii@ requires no buffering. When we pull a fold result from the result source, the combinator can pull the corresponding segment length from its argument source, then pull the corresponding number of elements from the element source, folding them in the process. 
...
\end{center}
\vspace{-1em}
The @folds_iio@ version can execute without buffering, because all the segment lengths and elements in the flow are available as sources, and the result of folding each segment can be pushed directly to the result sink. However, this version is not primitive as it can be expressed as the composition of @folds_iii@ and @drainP@ as shown above. We refer to a version that can be expressed in terms of a composition with @drainP@ as an \emph{active version} because its form implies computation rather than being an \emph{inactive} transformation on sources on sinks.
