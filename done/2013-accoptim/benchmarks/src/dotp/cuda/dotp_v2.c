 ...
 * CUBLAS version
 */
float dotp_cublas(cublasHandle_t hdl, int n, float* d_x, int incx, float* d_y, int incy)
{
     ...
     */
    status = cublasSdot(hdl, n, d_x, incx, d_y, incy, &dot);
...
int main(int argc, char **argv)
{
    ...
    // Allocate device vectors; initialise
    if (cudaSuccess != cudaMalloc((void**) &d_x, n * sizeof(float))
        ||
        cudaSuccess != cudaMalloc((void**) &d_y, n * sizeof(float))
    ) {
    ...
    const float dot = dotp_cublas(handle, n, d_x, 1, d_y, 1);
     ...
     */
    for (i = 0; i < 100; ++i) {
        dotp_cublas(handle, n, d_x, 1, d_y, 1);
    }
    ...
    free(h_x);
    free(h_y);
    if (cudaSuccess != cudaFree(d_x)
        ||
        cudaSuccess != cudaFree(d_y)
    ) {
    ...
    if (CUBLAS_STATUS_SUCCESS != cublasDestroy(handle)) {
        fprintf(stderr, "!!!! CUBLAS finalisation error\n");
        exit(EXIT_FAILURE);
    }
